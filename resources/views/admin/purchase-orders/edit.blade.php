@php
    $initialItems = $purchaseOrder->items->map(fn ($i) => [
        'itemable_type' => $i->itemable_type === \App\Models\Product::class ? 'product' : 'consumable',
...
<x-layouts.app title="Edit Purchase Order" :heading="'Edit '.$purchaseOrder->po_number">
    <div x-data="purchaseOrderForm($el.querySelector('[data-purchase-order-data]').textContent)" class="mx-auto max-w-3xl">
        ...
        <x-card>
            <form method="POST" action="{{ route('admin.purchase-orders.update', $purchaseOrder) }}" class="space-y-6">
                @csrf
                @method('put')
                ...
                <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                    ...
                    <div>
                        ...
                        <x-text-input id="expected_date" type="text" name="expected_date" data-flatpickr :value="old('expected_date', $purchaseOrder->expected_date?->format('Y-m-d'))" />
                ...
                <div>
                    ...
                    <x-textarea id="notes" name="notes">{{ old('notes', $purchaseOrder->notes) }}</x-textarea>
                    <x-input-error :messages="$errors->get('notes')" />
                </div>

                <div class="flex justify-end gap-3">
                    <x-button as="a" href="{{ route('admin.purchase-orders.show', $purchaseOrder) }}" variant="secondary">Cancel</x-button>
