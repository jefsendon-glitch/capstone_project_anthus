    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div class="space-y-6">
            ...
            @if(in_array($order->status, ['confirmed', 'out_for_delivery']))
                <x-card class="border-success-100 dark:border-success-500/20"><h2 class="font-heading text-base font-bold text-slate-900 dark:text-white">Complete Delivery</h2><p class="mt-1 text-xs text-slate-400">Records the sale, adjusts inventory, and closes this order.</p><form method="POST" action="{{ route('deliveries.fulfill', $order) }}" class="mt-4 space-y-3">@csrf @if($order->payment_method)<div class="rounded-xl bg-slate-50 p-3 text-sm dark:bg-white/5"><span class="text-slate-500 dark:text-slate-400">Customer selected:</span> <span class="font-semibold text-slate-900 dark:text-white">{{ $order->payment_method === 'loan' ? 'Credit' : 'Cash' }}</span></div>@else <x-select name="payment_method" required><option value="cash">Cash payment</option><option value="loan">Loan / credit payment</option></x-select>@endif <x-button type="submit" class="w-full"><x-icon name="check-circle" class="size-4" /> Mark as Delivered</x-button></form></x-card>
            @endif
